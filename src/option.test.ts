...
describe("Option", () => {
  it("should let you map across a Some", () => {
    ...
    expect(val.map((x) => x * 2)).toEqual(new Some(20))
  })

  it("should let you filter a Some", () => {
    const some = new Some(10)
    expect(some.filter((x) => x === 30)).toEqual(new None())

    expect(some.filter((x) => x === 10)).toEqual(new Some(10))
  })
  ...
  it("should let you chain filter, map, etc", () => {
    ...
    expect(updatedName).toEqual(new Some("MY"))

    const none = updatedName.filter((name) => name === "My")
  ...
  it("should let you map across a None", () => {
    ...
    expect(none.map((x: number) => x + 2)).toEqual(new None())
  })

  it("should filter a None", () => {
    const none: Option<string> = new None()
    expect(none.filter((x) => x !== "hey!")).toEqual(new None())
  })
